...
//add unit test for batchcopy
func TestBatchCopy(t *testing.T) {
    ctx := context.Background()
    r := fstest.NewRun(t)
    defer r.Finalise()
    file1 := r.WriteFile("file1", "batch copy1", t1)
    r.Mkdir(ctx, r.Fremote)

    var tempPairs fs.BatchCopyPairs
    var srcItem fs.BatchCopyItem
    var dstItem fs.BatchCopyItem
    ...
    dstItem = fs.BatchCopyItem{
        FsPath: r.Fremote.Name(),
        FsInst: r.Fremote,
    }

    tempPairs = append(tempPairs, &fs.BatchCopyPair{
        FsSrc: srcItem,
        FsDst: dstItem,
    })
    restList := BatchCopy(ctx, tempPairs, false)

    var err error
    if restList != nil {
        err = errors.New("list should be nil")
    }
    require.NoError(t, err)
